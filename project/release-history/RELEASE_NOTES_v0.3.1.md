...
> **Current formal release / 当前正式版本（2026-09-08）：** This note describes the fixed `v0.3.1` release. A later [P12 page note](../../P12_CORRIGENDUM.md) withdraws the old cross-estimand opposing-subclaim rationale without altering the fixed 13+1 machine classification or assigning a new P12 verdict.
>
> **中文提示：** 本文记录当前正式版`v0.3.1`的固定发布内容。原 P12 跨估计量起点差理由已由[后续P12论证说明](../../P12_CORRIGENDUM.md)撤回；13+1机器分类及固定发布包不变，也不据此作新的P12判定。
...
- P12 was labeled partially consistent. The original rationale based on comparing size onsets across different estimands is superseded by a later P12 page note; the published label is retained without a new whole-proposition decision.
...
- P12当时标为部分一致；原先借不同估计量的起点差支持这一标签的理由已由后续P12论证说明撤回。已发布标签继续保留，但不构成新的整项判定；
- 整体不一致0项，未知0项。
