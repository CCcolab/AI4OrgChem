...
本导航把每项范围化科学判定连接到冻结数据卡、协议、机器结果和报告。“一致”只表示在声明受测域内与原著相应命题一致，不等于历史身份复现、普遍定律或机构认证。十四项是分层证据链，不代表十四次同等级独立量子化学复现。

## 分类汇总（v0.3.1已发布历史快照）

下列P12标签仅供版本追溯，不表示本次重新确认三分判定。当前证据解释见[P12勘误](../../P12_CORRIGENDUM.md#中文)；本地候选量化数值未作为本公开包的新增结果。
...
| P07 | “共轭稳定化—位阻去稳定化”二项叙事不足以解释受测机制。 | **一致** | 独立同哈密顿量五状态路径审计＋P04–P06命题综合；天真三项残差约0.336/0.334 Eh，加入桥接项或路径完整σ定义后严格闭合。 | [数据卡](P07/data-card.md) · [协议](P07/protocol.md) · [综合结果](P07/result.json) · [审计结果](P07/same-hamiltonian-audit-result.json) · [报告](P07/report.md) |
...
| P12 | 大轮烯趋近多烯能量行为；精确起点须按可比口径检验。 | **部分一致（v0.3.1历史标签）** | 原著六点表值通过算术/源账本核验，并非六点独立量化复算。CESE **N=16/18** 与ASE **N>30** 属不同估计量，其差异不是同口径反例；精确CESE起点尚未独立确认。 | [勘误](../../P12_CORRIGENDUM.md#中文) · [数据卡](P12/data-card.md) · [协议](P12/protocol.md) · [机器结果](P12/result.json) · [报告](P12/report.md) |
...
原著六点账本的算术和内部趋势已核验，独立ASE研究为大环定性方向提供不同估计量的旁证。但原著CESE/ΔEA语义的起点是N=16/18，ASE报告N>30；二者测量对象不同，既不能互作精确起点的独立复现，也不能把数值差当成反对子项。该历史标签的旧理由已失效，尚无完整同口径证据支持自动改判；详见[勘误](../../P12_CORRIGENDUM.md#中文)。
...
第一个验证器检查14项打包结果；第二个验证器从4个公开底层JSON重算P14的BLA/能量判据并检查输入和哈希。二者均不重跑高成本QM。跨命题总矩阵见[`manuscripts/P01-P14_evidence_matrix_zh-CN.md`](../../manuscripts/P01-P14_evidence_matrix_zh-CN.md)。
