...
The P12 label below is historical, not a reaffirmed current three-way verdict. See the [P12 evidence corrigendum](../../P12_CORRIGENDUM.md) for the current interpretation; no local candidate QM numbers are included in this public package.

- 13 consistent;
- 1 partially consistent: P12;
...
| P02 | Conjugation can accompany, rather than prevent, molecular distortion. | **Consistent** | A single published crystal-pair structure-spectrum association supports the scoped direction; it is not a causal test, and original CIF refinement was not repeated. | [card](P02/data-card.md) · [protocol](P02/protocol.md) · [result](P02/result.json) · [report](P02/report.md) |
...
| P05 | A pi-sigma source endpoint can be destabilizing. | **Consistent** | All six nonzero endpoints from 5 to 45 degrees are positive in the same Table 5-15 source-proxy continuation; 0 degrees remains the zero control. | [card](P05/data-card.md) · [protocol](P05/protocol.md) · [seven-angle result](P05/seven-angle-result.json) · [historical anchors](P05/result.jsonl) · [report](P05/report.md) |
...
| P07 | A two-term “conjugative stabilization versus steric destabilization” account is insufficient for the tested mechanism. | **Consistent** | Independent same-Hamiltonian five-state path audit plus P04-P06 synthesis; the naive three terms miss about 0.336/0.334 Eh, whereas an explicit bridge or path-complete sigma definition closes exactly. | [card](P07/data-card.md) · [protocol](P07/protocol.md) · [synthesis](P07/result.json) · [audit](P07/same-hamiltonian-audit-result.json) · [report](P07/report.md) |
...
| P09 | Aromatic and antiaromatic energies can be constructed from theoretical localized/virtual references. | **Consistent** | Cyclobutadiene ADE is **+53.822467 kcal/mol**. Benzene ESE is **-37.412764 kcal/mol**, `1.112764 kcal/mol` (about `3.07%`) from the monograph's `-36.3`. The ordinary-state sign is robust, but the method span is **41.0077 kcal/mol** and the CIPSI lane is not fully converged; this is source-aligned support, not a method-independent or digit-for-digit reproduction. | [card](P09/data-card.md) · [protocol](P09/protocol.md) · [result](P09/result.json) · [report](P09/report.md) |
...
| P11 | Furan requires an LDE treatment, while substituted-benzene conjugative and inductive components must be separated. | **Consistent** | P11-A furan LDE is consistent. In P11-B, source-2007 state-specific planar restricted optimizations give **CE=+1.173122** and **IE=+0.490079 kcal/mol**, versus about **+1.2** and **+0.49** in the source; 13/13 states converged. The old fixed-geometry opposite sign is retained as a superseded audit diagnostic. | Furan: [card](P11/furan-data-card.md) · [protocol](P11/furan-protocol.md) · [result](P11/furan-result.json) · [report](P11/furan-report.md). Substituent: [card](P11/substituent-data-card.md) · [protocol](P11/substituent-protocol.md) · [result](P11/substituent-result.json) · [state records](P11/optimized-states/) · [fixed-proxy audit](P11/substituent-fixed-geometry-audit-result.json) · [report](P11/substituent-report.md) |
| P12 | Large annulenes approach polyene-like energetic behavior; the precise size onset requires a comparable test. | **Partially consistent (v0.3.1 historical label)** | The six monograph values have an arithmetic/source-ledger check, not six-point independent QM reproduction. CESE **N=16/18** and ASE **N>30** refer to different estimands; their onset difference is not a same-estimand counterexample. The precise CESE onset remains independently unconfirmed. | [corrigendum](../../P12_CORRIGENDUM.md) · [card](P12/data-card.md) · [protocol](P12/protocol.md) · [result](P12/result.json) · [report](P12/report.md) |
...
| P14 | Pi interaction can contribute to distortion in a strained aromatic system. | **Consistent** | The repaired gate rejects the old STO-3G technical pilot. Qualified B3LYPG/6-31G(d) G/PLG optimizations give `dΔr=0.172204 Å` and `67.679719 kcal/mol`; both gradients are below `0.002 Eh/Å`, all eligibility checks pass, and the structure/energy residuals are within the preregistered tolerances. Scope remains one C12H6 planar-D3h five-parameter source-proxy system. | [card](P14/data-card.md) · [protocol](P14/protocol.md) · [result](P14/result.json) · [lower-level records](P14/processed/) · [inputs](P14/inputs/README.md) · [report](P14/report.md) |
