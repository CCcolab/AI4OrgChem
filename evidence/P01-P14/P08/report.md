...
本结果在当前冻结的公开条件source-aligned协议内支持原著关于反式-1,3-丁二烯的三个范围化命题。项目未使用原著程序代码，也不宣称与原厂PC-GAMESS实现逐项同一。同一实现采用不同普通RKS初始密度的复算只证明收敛稳健性，不是第二套程序复现。旧CASCADE常规热化学结论仅作外部参考，不参与本判定。本结果不证明所有分子的普遍共轭去稳定化定律，也不是生产标签。
