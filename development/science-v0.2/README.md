...
Gate V2-0 contracts, machine-readable records, environment identity, and current blockers for
the planned V0.2 evidence-strengthening work. The frozen V0.1 P01-P14 classifications are not
changed by this snapshot.

本目录是**开发快照**，不是新的科学结论版本。它公开 Gate V2-0 合同、机器记录、环境身份和
剩余阻塞；不会修改 V0.1 已冻结的 P01-P14 十四项结论。

## Current state / 当前状态

```text
GATE_V2_0: IN_PROGRESS_NOT_PASSED
QP22_CIPSI_ENVIRONMENT: PROVISIONED_AND_NON_CBD_SMOKE_VERIFIED
WP1_CBD_SCIENTIFIC_CALCULATION: NOT_AUTHORIZED
WP3_HESSIAN_CALCULATION: NOT_AUTHORIZED
...
upgrade any scientific evidence grade.

Quantum Package 记录只覆盖隔离构建和官方 H2/拉伸 B2 烟测；它不是环丁二烯结果，不验证
WP1 估计量，也不提升任何科学证据等级。
